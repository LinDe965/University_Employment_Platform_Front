<template>
    <div>
        ...
        <div class="container">
            ...
            <el-table :data="studentFavoriteData" border class="table" ref="multipleTable"
                ...
                <!--收藏时间-->
                <el-table-column prop="studentFavoritesTime" align="center" label="收藏时间"></el-table-column>

                <!--收藏状态-->
                <el-table-column prop="favoritesStatus" align="center" label="面试状态">
                    <template #default="scope">
                        <el-tag :type="
                                     scope.row.favoritesStatus === 1
                                     ? 'danger'
                                     : scope.row.favoritesStatus === 0
                                     ? 'success'
                                     : 'success'
                                     "
...
import {
    fetchStudentFavoriteLikeByStudent,
    fetchUpdateDeliverStatusByCompanyHr,
    fetchUpdateFavoriteStatusByStudent,
    getStudentFavoriteByStudentId
} from "../../api/index";
...
export default {
    ...
    methods: {
        state(row) {
            let user_state = row.favoritesStatus;
            if (user_state === 1) {
                return '喜欢'
            } else {
                return '不喜欢'
    ...
    setup() {
        ...
        const studentFavoriteData = ref([]);
        const pageTotal = ref(0);

        // 获取表格数据
        const getData = () => {
            const studentId = Cookies.get("studentId");
            getStudentFavoriteByStudentId(studentId).then((res) => {
                console.log(res.data);
                studentFavoriteData.value = res.data;
        ...
        // 查询操作
        const handleSearch = () => {
            fetchStudentFavoriteLikeByStudent(query).then((res) => {
                console.log(res.data);
                studentFavoriteData.value = res.data;
                //pageTotal.value = res.pageTotal || 50;
            });
        ...
        const handleDelete = (index) => {
            ...
            ElMessageBox.confirm("确定要删除吗？", "提示", {
                ...
                .then(() => {
                    ...
                    studentFavoriteData.value.splice(index, 1);
                })
                .catch(() => {
                });
        };
        ...
        const saveEdit = () => {
            ElMessageBox.confirm("确定要修改吗？", "提示", {type: "warning",}).then(() => {
                fetchUpdateFavoriteStatusByStudent(form).then((res) => {
                    console.log(res)
                    if (res.code == 20031) {
                        ElMessage.success(`修改第 ${idx + 1} 行成功`);
                        Object.keys(form).forEach((item) => {
                            studentFavoriteData.value[idx][item] = form[item];
                        });
                        getData();
                    } else {
