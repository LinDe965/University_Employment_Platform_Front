<template>
    <div>
        ...
        <div class="container">
            ...
            <el-table :data="studentAdmitPositionData" border class="table" ref="multipleTable"
                ...
                <!--入职时间-->
                <el-table-column prop="signingTime" align="center" label="录用时间"></el-table-column>
                <!--入职状态-->
                <el-table-column prop="interviewStatus" align="center" label="是否录用(hr)">
                ...
                <el-table-column prop="interviewStudentStatus" align="center" label="签约状态(stu)">
                    <template #default="scope">
                        <el-tag :type="
...
<!--            </div>-->
        ...
        <el-dialog title="编辑" v-model="editVisible" width="30%">
            <el-switch
                v-model="form.signingStudentStatus"
                active-color="#13ce66"
...
import {
    fetchStudentSigningLikeByStudent,
    fetchUpdateSigningStudentStatusByStudent,
    getStudentSigningByStudentId
} from "../../api/index";
...
export default {
    ...
    setup() {
        ...
        const studentAdmitPositionData = ref([]);
        const pageTotal = ref(0);

        // 获取表格数据
        const getData = () => {
            const studentId = Cookies.get("studentId");
            getStudentSigningByStudentId(studentId).then((res) => {
                console.log(res.data);
                studentAdmitPositionData.value = res.data;
        ...
        // 查询操作
        const handleSearch = () => {
            fetchStudentSigningLikeByStudent(query).then((res) => {
                console.log(res.data);
                studentAdmitPositionData.value = res.data;
            });
        };
        ...
        const handleDelete = (index) => {
            ...
            ElMessageBox.confirm("确定要删除吗？", "提示", {
                ...
                .then(() => {
                    ...
                    studentAdmitPositionData.value.splice(index, 1);
                })
                .catch(() => {
                });
        };
        ...
        const saveEdit = () => {
            ElMessageBox.confirm("确定要通过吗？", "提示", {type: "warning",}).then(()=>{
                fetchUpdateSigningStudentStatusByStudent(form).then((res) => {
                    console.log(res)
                    if (res.code == 20031) {
                        ElMessage.success(`修改第 ${idx + 1} 行成功`);
                        Object.keys(form).forEach((item) => {
                            studentAdmitPositionData.value[idx][item] = form[item];
                        });
                        getData();
                    } else {
